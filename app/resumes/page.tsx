...
export default function ResumesPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null);
    try {
      const data = await ApiService.uploadResumeVersion(resumeId, file);
  ...
  const selectedResume = resumes.find(r => r.id === selectedVersionId);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Upload new resume files to parse them into structured profiles and compare version histories.
          </p>
        </div>
      </div>
            ...
            <label className="border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:border-blue-500 transition-all bg-zinc-50/50 dark:bg-zinc-950/20 group">
              {uploading ? (
                <div className="space-y-2 flex flex-col items-center">
                  ...
                  <span className="text-xs text-zinc-500 font-medium">Local Qwen parsing file...</span>
                </div>
              ) : (
                <div className="space-y-2 flex flex-col items-center">
                  <Upload className="h-8 w-8 text-zinc-400 group-hover:text-blue-500 transition-colors" />
                  <span className="text-xs text-zinc-700 dark:text-zinc-300 font-semibold">Choose PDF/DOCX file</span>
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto pr-1">
                {resumes.map((res) => (
                  ...
                  >
                    ...
                    <p className="text-xs truncate font-medium">{res.original_filename}</p>
                  </button>
                ))}
              </div>
            )}
          ...
          {selectedResume ? (
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-6">
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <h2 className="text-xl font-bold text-zinc-950 dark:text-zinc-50">
                        {selectedResume.parsed_resume?.personal_info?.name || "Dev Sharma"}
                      </h2>
                ...
                {/* Profile Links */}
                {selectedResume.parsed_resume?.personal_info?.links && selectedResume.parsed_resume.personal_info.links.length > 0 && (
                  <div className="flex flex-wrap gap-3 pt-2">
                    {selectedResume.parsed_resume.personal_info.links.map((link: string, idx: number) => (
                      <a
                        key={idx}
              ...
              {/* Skills Section */}
              {selectedResume.parsed_resume?.skills && selectedResume.parsed_resume.skills.length > 0 && (
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedResume.parsed_resume.skills.map((skill: string) => (
                      <span
                        key={skill}
              ...
              {/* Experience Section */}
              {selectedResume.parsed_resume?.experience && selectedResume.parsed_resume.experience.length > 0 && (
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-6">
                  ...
                  <div className="space-y-6 divide-y divide-zinc-100 dark:divide-zinc-800">
                    {selectedResume.parsed_resume.experience.map((exp: any, idx: number) => (
                      <div key={idx} className={`space-y-2 ${idx > 0 ? "pt-6" : ""}`}>
              ...
              {/* Projects Section */}
              {selectedResume.parsed_resume?.projects && selectedResume.parsed_resume.projects.length > 0 && (
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-6">
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
                  ...
                  <div className="space-y-6 divide-y divide-zinc-100 dark:divide-zinc-800">
                    {selectedResume.parsed_resume.projects.map((proj: any, idx: number) => (
                      <div key={idx} className={`space-y-2 ${idx > 0 ? "pt-6" : ""}`}>
                        <div className="flex justify-between items-center">
                          <h4 className="font-bold text-zinc-900 dark:text-white">{proj.title}</h4>
                          {proj.links && proj.links.length > 0 && (
                            <a
                              href={proj.links[0].startsWith("http") ? proj.links[0] : `https://${proj.links[0]}`}
                              target="_blank"
                              rel="noreferrer"
                        ...
                        {proj.technologies && proj.technologies.length > 0 && (
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {proj.technologies.map((tech: string) => (
                              <span key={tech} className="bg-blue-50/50 dark:bg-blue-950/20 text-blue-700 dark:text-blue-400 text-[10px] px-2 py-0.5 rounded font-mono">
                                {tech}
                              </span>
                            ))}
                          </div>
              ...
              {/* Education Section */}
              {selectedResume.parsed_resume?.education && selectedResume.parsed_resume.education.length > 0 && (
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-6">
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-3">
                  ...
                  <div className="space-y-6 divide-y divide-zinc-100 dark:divide-zinc-800">
                    {selectedResume.parsed_resume.education.map((edu: any, idx: number) => (
                      <div key={idx} className={`space-y-1.5 ${idx > 0 ? "pt-6" : ""}`}>
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-1">
                          <h4 className="font-bold text-zinc-900 dark:text-white">
              ...
              {selectedResume.parsed_resume?.certifications && selectedResume.parsed_resume.certifications.length > 0 && (
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm space-y-4">
                  <h3 className="text-sm font-bold text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                  ...
                  <ul className="space-y-2 list-disc list-inside text-sm text-zinc-600 dark:text-zinc-300">
                    {selectedResume.parsed_resume.certifications.map((cert: any, idx: number) => (
                      <li key={idx} className="leading-relaxed">
                        <strong className="text-zinc-800 dark:text-zinc-200">{cert.title}</strong>
                        {cert.date && <span className="text-zinc-400 text-xs ml-2">({cert.date})</span>}
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-2xl shadow-sm text-center py-36 text-zinc-400 flex flex-col items-center justify-center">
              ...
              <h4 className="text-base font-semibold text-zinc-900 dark:text-white mb-1">No Version Selected</h4>
              <p className="text-sm max-w-sm">Select an uploaded version from the left panel, or upload a new file to parse your profile data.</p>
            </div>
          )}
        </div>
